...
class Road:
    ...
    Attributes:
        ...
        yaw_deg (Optional[List[float]]): Per-point yaw values in degrees.
        pitch_deg (Optional[List[float]]): Per-point pitch values in degrees.
        roll_deg (Optional[List[float]]): Per-point roll values in degrees.
    """
    ...
    def __post_init__(self):
        ...
        length = 0.0
        for (x0, _, z0), (x1, _, z1) in zip(self.points_m, self.points_m[1:]):
            length += math.hypot(x1 - x0, z1 - z0)
        if length == 0.0:
            raise ValueError("Road polyline length must be > 0")

        for attr_name in ("yaw_deg", "pitch_deg", "roll_deg"):
            values = getattr(self, attr_name)
            if values is not None and len(values) != len(self.points_m):
                raise ValueError(f"{attr_name} must have the same length as points_m")
